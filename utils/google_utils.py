...
def gdrive_download(id='1HaXkef9z6y5l4vUnCYgdmEAj61c6bfWO', name='coco.zip'):
    ...
    s = ["curl -x http://127.0.0.1:1087 -c ./cookie -s -L \"https://drive.google.com/uc?export=download&id=%s\" > /dev/null" % id,
         "curl -x http://127.0.0.1:1087 -Lb ./cookie -s \"https://drive.google.com/uc?export=download&confirm=`awk '/download/ {print $NF}' ./cookie`&id=%s\" -o %s" % (
             id, name),
         'rm ./cookie']
    ...
    if not os.path.exists(name):  # file size < 40MB
        s = 'curl  -x http://127.0.0.1:1087 -f -L -o %s https://drive.google.com/uc?export=download&id=%s' % (name, id)
        r = os.system(s)
